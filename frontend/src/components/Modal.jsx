...
export default function Modal({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent
        className={cn("bg-card/95 backdrop-blur-xl border border-border/40 shadow-2xl rounded-2xl sm:rounded-3xl p-0 overflow-hidden",
          sizeClasses[size],
          className
        )}
      >
        ...
        {footer && (
          <DialogFooter className="px-4 sm:px-6 md:px-8 pb-4 sm:pb-6 md:pb-8 pt-3 sm:pt-4 border-t border-border/50">
            {footer}
          </DialogFooter>
        )}
      </DialogContent>
