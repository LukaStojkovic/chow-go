...
import {
  ...
  useCourierLocation,
} from "@/contexts/CourierLocationContext";

function LocationPermissionNotice() {
  const { isDenied, isUnsupported, error } = useCourierLocation();
  if (!isDenied && !isUnsupported) return null;

  return (
    <div className="flex items-start gap-2.5 border-b border-destructive/30 bg-destructive/10 px-4 py-3">
...
export default function CourierLayout() {
  ...
  const isAvailable = authUser?.courier?.isAvailable ?? false;

  const { courierOrders: activeData } = useGetCourierOrders("active");
  const activeOrder = activeData?.data?.orders?.[0] ?? null;

  const isDeliveryPage = location.pathname.startsWith("/courier/delivery");

  const shouldTrackLocation = Boolean(isAvailable || activeOrder);
        ...
        <main className="flex min-w-0 flex-1 flex-col h-screen overflow-hidden bg-muted ">
          <header className="flex shrink-0 items-center justify-between border-b border-border bg-card p-4 lg:hidden ">
            ...
            <div className="flex items-center gap-3">
              <div
                className={`h-3 w-3 rounded-full ${isAvailable || activeOrder ? "bg-primary animate-pulse" : "bg-secondary "}`}
              />
              <button
