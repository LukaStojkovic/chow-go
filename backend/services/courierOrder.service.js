...
export async function listCourierOrders({ courierId, status, page = 1, limit = 20 }) {
  const pageNum = parseInt(page);
  const limitNum = parseInt(limit);
  const skip = (pageNum - 1) * limitNum;
...
export async function acceptOrderOperation({ orderId, courierUserId }) {
  ...
  if (!courier.isOnline || !courier.isAvailable) {
    throw new AppError("You must be online and available to accept orders", 400);
  }
...
export async function cancelAssignedOrderOperation({ orderId, courierUserId, reason }) {
  const courier = await getCourierByUserId(courierUserId);
  ...
  await socketService.emitOrderCourierUnassigned(order, reason || "Cancelled by courier");
...
export async function updateCourierLocation({ courierUserId, lat, lng, orderId }) {
  const courier = await getCourierByUserId(courierUserId);
  ...
  if (orderId) {
    const order = await Order.findById(orderId);
    if (order && order.courier?.toString() === courier._id.toString()) {
      await socketService.emitCourierLocationUpdated(order, updatedCourier);
    }
  }
