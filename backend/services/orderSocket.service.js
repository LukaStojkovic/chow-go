...
export async function emitOrderAssigned(order) {
  try {
    ...
    socketServer.emitToRestaurant(populatedOrder.restaurant._id, "order:assigned", {
      order: populatedOrder,
...
export async function emitOrderCourierUnassigned(order, reason) {
  try {
    ...
    socketServer.emitToRestaurant(populatedOrder.restaurant._id, "order:courier_unassigned", {
      order: populatedOrder,
      reason,
...
export async function emitOrderPickedUp(order) {
  try {
    ...
    socketServer.emitToCustomer(populatedOrder.customer._id, "order:picked_up", {
      order: populatedOrder,
      message: "Courier picked up your order",
    });
    socketServer.emitToRestaurant(populatedOrder.restaurant._id, "order:picked_up", {
      order: populatedOrder,
      message: "Order picked up by courier",
    });
    if (populatedOrder.courier?._id) {
      socketServer.emitToCourier(populatedOrder.courier._id, "order:picked_up", {
        order: populatedOrder,
...
export async function emitOrderInTransit(order) {
  try {
    ...
    socketServer.emitToCustomer(populatedOrder.customer._id, "order:in_transit", {
      order: populatedOrder,
      message: "Courier is on the way",
    });
    socketServer.emitToRestaurant(populatedOrder.restaurant._id, "order:in_transit", {
      order: populatedOrder,
      message: "Courier started delivery",
    });
    if (populatedOrder.courier?._id) {
      socketServer.emitToCourier(populatedOrder.courier._id, "order:in_transit", {
        order: populatedOrder,
...
export async function emitOrderDelivered(order) {
  try {
    ...
    socketServer.emitToCustomer(populatedOrder.customer._id, "order:delivered", {
      order: populatedOrder,
      message: "Order delivered",
    });
    socketServer.emitToRestaurant(populatedOrder.restaurant._id, "order:delivered", {
      order: populatedOrder,
      message: "Order delivered to customer",
    });
    if (populatedOrder.courier?._id) {
      socketServer.emitToCourier(populatedOrder.courier._id, "order:delivered", {
        order: populatedOrder,
...
export async function emitCourierLocationUpdated(order, courier) {
  try {
    ...
    socketServer.emitToCustomer(populatedOrder.customer._id, "courier:location", payload);
    socketServer.emitToRestaurant(populatedOrder.restaurant._id, "courier:location", payload);
    if (populatedOrder.courier?._id) {
      socketServer.emitToCourier(populatedOrder.courier._id, "courier:location", payload);
    }
  } catch (error) {
